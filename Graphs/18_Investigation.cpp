...
int main() {
    ...
    vector<node> dist(n);
    for(int i = 0; i < n; i++) {
        dist[i] = {inf, 0, (int)1e9, (int)-1e9};
    }
    dist[0] = {0, 1, 0, 0};

    set<pair<long long, int>> s;
    s.emplace(dist[0].cost, 0);
    ...
    while(!s.empty()) {
        ...
        for(auto &[v, c] : g[u]) {
            long long nc = dist[u].cost + c;

            if(nc < dist[v].cost) {
                dist[v].cost = nc;
                dist[v].paths = dist[u].paths;
                dist[v].mn_cnt = 1 + dist[u].mn_cnt;
                dist[v].mx_cnt = 1 + dist[u].mx_cnt;
                s.emplace(dist[v].cost, v);
            } else if(nc == dist[v].cost) {
                dist[v].paths += dist[u].paths;
                cmin(dist[v].mn_cnt, 1 + dist[u].mn_cnt);
                cmax(dist[v].mx_cnt, 1 + dist[u].mx_cnt);
                s.emplace(dist[v].cost, v);
            }
        }
    }
    ...
    auto res = dist[n - 1];
    cout << res.cost << " " << res.paths << " " << res.mn_cnt << " " << res.mx_cnt << '\n';
